...
G4LambdaCDecayWithSpin::G4LambdaCDecayWithSpin(const G4String& processName):G4Decay(processName)
{
...
G4LambdaCDecayWithSpin::~G4LambdaCDecayWithSpin(){}

G4VParticleChange* G4LambdaCDecayWithSpin::PostStepDoIt(const G4Track& aTrack, const G4Step& aStep)
{
...
// get parent_polarization
  ...
  //G4cout<<"Now polarization is: "<<parent_polarization<<G4endl;
  ...
  if(parent_polarization == G4ThreeVector(0,0,0))
  {
    ...
    if (field) {
       ...
       if (B.mag2() > 0.) parent_polarization =
                                 Spin_Precession(aStep,B,aTrack.GetLocalTime());

    }
  }

  // section to check the field:
  G4ThreeVector beta = aTrack.GetMomentum()/aTrack.GetTotalEnergy();
  G4ThreeVector position = aStep.GetPostStepPoint()->GetPosition();
  //G4cout<<"actually traveled: "<<position.mag()<<" should be: "<<beta.mag()*3.0e2*aTrack.GetLocalTime()<<G4endl;

  AllPixRunAction* runaction = (AllPixRunAction*)G4RunManager::GetRunManager()->GetUserRunAction();
  runaction->lambdaC_polarization = parent_polarization;
...
G4VParticleChange* G4LambdaCDecayWithSpin::AtRestDoIt(const G4Track& aTrack, const G4Step& aStep)
{
...
G4ThreeVector G4LambdaCDecayWithSpin::Spin_Precession( const G4Step& aStep,
